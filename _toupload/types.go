...
type ActionDef struct {
	...
	Func   ActionHandler // Function to handle the action
...
type IModule interface {
	GetName() string // Name returns the name of the module
	Init() error     // Init initializes the module
	Stop() error     // Stop stops the module

	// Actions
	RegisterAction(action ActionDef) error
	UnregisterAction(name string) error
	GetActions() []ActionDef
	GetAction(name string) (ActionDef, bool)

	// Handlers
	RegisterHandler(name string, handler ActionHandler) error
	UnregisterHandler(name string) error
	GetHandler(name string) (ActionHandler, bool)
	GetHandlers() map[string]ActionHandler
}
