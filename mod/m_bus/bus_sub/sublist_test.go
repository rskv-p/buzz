// file: buzz/mod/m_bus/bus_sub/sublist_test.go
...
func TestMatchExactSubscription(t *testing.T) {
	...
	sub := &typ.Subscription{Subject: []byte("foo.bar.baz"), Queue: []byte("queue1"), Client: nil}

	// Insert the subscription
	sublist.Insert(sub)

	// Log all subscriptions after insertion
	x_log.Info("All subscriptions after insertion:")
	for key, subs := range sublist.ExactCache {
		x_log.Info("Exact Cache:", "Subject =", key, "Subscriptions =", subs)
	}
	for key, subs := range sublist.WildcardCache {
		x_log.Info("Wildcard Cache:", "Subject =", key, "Subscriptions =", subs)
	}
	...
	x_log.Info("All subscriptions after matching:")
	for key, subs := range sublist.ExactCache {
		x_log.Info("Exact Cache:", "Subject =", key, "Subscriptions =", subs)
	}
	for key, subs := range sublist.WildcardCache {
...
func TestRemoveSubscription(t *testing.T) {
	...
	x_log.Info("All subscriptions after removal:")
	for key, subs := range sublist.ExactCache {
		x_log.Info("Exact Cache:", "Subject =", key, "Subscriptions =", subs)
	}
	for key, subs := range sublist.WildcardCache {
...
func TestMatchWildcardSubscription(t *testing.T) {
	...
	sub1 := &typ.Subscription{Subject: []byte("foo.bar.baz"), Queue: []byte("queue1"), Client: nil}
	sub2 := &typ.Subscription{Subject: []byte("foo.x.baz"), Queue: []byte("queue2"), Client: nil}
	sub3 := &typ.Subscription{Subject: []byte("foo.baz.qux"), Queue: []byte("queue3"), Client: nil}
	...
	x_log.Info("Matching with filter 'foo.>'...")
	result := sublist.MatchSubjectWithWildcard(filter)

	// Log results
	x_log.Info("Matching result:", result.Psubs)

	// Check the number of matches
	if len(result.Psubs) != 3 {
		t.Fatalf("expected 3 matches for filter 'foo.>', got %d", len(result.Psubs))
	} else {
		x_log.Info("Match for filter 'foo.>' passed with", len(result.Psubs), "match(es)")
	}

	// Verify that the correct subscriptions matched
	x_log.Info("Verifying matched subscriptions...")
	...
	for _, expectedSubject := range expectedSubjects {
		...
		if !found {
			...
			x_log.Info("Correct subscription matched for subject:", expectedSubject)
		}
	}
}
...
func TestInsertMultipleSubscriptions(t *testing.T) {
	...
	sub1 := &typ.Subscription{Subject: []byte("foo.bar.baz"), Queue: []byte("queue1"), Client: nil}
	sub2 := &typ.Subscription{Subject: []byte("foo.bar.baz"), Queue: []byte("queue2"), Client: nil}
	sub3 := &typ.Subscription{Subject: []byte("foo.baz.bar"), Queue: []byte("queue3"), Client: nil}
...
// 	sub1 := &typ.Subscription{Subject: []byte("foo.bar.baz"), Queue: []byte("queue1"), Client: nil}
// 	sub2 := &typ.Subscription{Subject: []byte("foo.baz.bar"), Queue: []byte("queue2"), Client: nil}
// 	sub3 := &typ.Subscription{Subject: []byte("bar.foo.baz"), Queue: []byte("queue3"), Client: nil}
// 	sub4 := &typ.Subscription{Subject: []byte("baz.foo.bar"), Queue: []byte("queue4"), Client: nil}
...
// 	// Log all subscriptions after insertion
// 	x_log.Printf("All subscriptions after insertion:")
// 	for key, subs := range sublist.ExactCache {
// 		x_log.Printf("Exact Cache: Subject = %s, Subscriptions = %+v", key, subs)
// 	}
...
// 	x_log.Printf("All subscriptions after eviction:")
// 	for key, subs := range sublist.ExactCache {
// 		x_log.Printf("Exact Cache: Subject = %s, Subscriptions = %+v", key, subs)
// 	}
...
// 	} else {
// 		x_log.Printf("Correct eviction: sub1 (foo.bar.baz) was evicted from cache")
// 	}
// }
