// file:buzz/mod/m_bus/bus_auth/store.go
...
func InitDB() error {
	...
	//cfg := cfg.Cfg.AuthConfig
	// Initialize SQLite database connection
	var err error
	db, err = gorm.Open(sqlite.Open(configAuth.DBpath), &gorm.Config{})
	if err != nil {
		x_log.Error("Failed to connect to database", configAuth.DBpath, err)
		return fmt.Errorf("failed to connect to database: %v", err)
	}
	x_log.Info("Successfully connected to the SQLite database", configAuth.DBpath)

	// Migrate the schema
	err = db.AutoMigrate(&User{}, &Permission{}, &Group{}, &ServiceAccount{}, &UserPermission{})
	if err != nil {
		x_log.Error("Failed to migrate database schema", err)
		return fmt.Errorf("failed to migrate database schema: %v", err)
	}
	x_log.Info("Database schema migration completed successfully")
...
func createDefaultAdmin() error {
	...
	err := db.Where("username = ?", configAuth.AdminLogin).First(&admin).Error
	if err == nil {
		// Admin user already exists, skip creation
		x_log.Info("Admin user already exists, skipping creation", configAuth.AdminLogin)
		return nil
	}

	// Hash the password before storing it
	hashedPassword, err := bcrypt.GenerateFromPassword([]byte(configAuth.AdminPassword), bcrypt.DefaultCost)
	if err != nil {
		x_log.Error("Failed to hash password for admin user", configAuth.AdminLogin, err)
		return fmt.Errorf("failed to hash password: %v", err)
	}

	// Create default admin user
	admin = User{
		Username: configAuth.AdminLogin,
		Password: string(hashedPassword),
	}

	// Create the admin permission (full access)
	adminPermission := Permission{Name: configAuth.AdminLogin}
	if err := db.Create(&adminPermission).Error; err != nil {
		x_log.Error("Failed to create admin permission", err)
	...
	if err := db.Create(&admin).Error; err != nil {
		x_log.Error("Failed to create admin user", configAuth.AdminLogin, err)
		return fmt.Errorf("failed to create admin user: %v", err)
	}

	x_log.Info("Successfully created default admin user", configAuth.AdminLogin)
